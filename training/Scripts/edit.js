...
var themeFlag = 0;
var lexicList = {};
var morphologyWord = {}, morphologyAnswer = {}, morphologyAnswer2 = {}, morphologyAnswer3 = {}, morphologyAnswer4 = {}; //3 ответ //4 ответ


var syllMirror = (function () {
    var self, exercises, step = 0;
    ...
    return {
        initialize: function () {
            ...
            self.setUpListeners();
            self.getPartsLists();
            self.loadLevel(document.getElementsByClassName("disabledClass")[0].getAttribute("data-level"));
            self.morphemsDisplay(document.getElementsByClassName("disabledClass")[0].getAttribute("data-id"), document.getElementsByClassName("disabledClass")[0].getAttribute("data-level"));
            abcdef();
            $("h1").slice(0, 1).click();
            //document.click();
        ...
        morphemsDisplay: function(oh, realLevel) {
        ...
        if (realLevel === '2') {
            ...
            if (morphologyAnswer3.word !== undefined && morphologyAnswer4.word !== undefined) {
                $('.lexic4_12').slice(lexicList[0].lexic12 - 1, lexicList[0].lexic12).prop("checked", true);
                $('.lexic4_13').slice(lexicList[0].lexic13 - 1, lexicList[0].lexic13).prop("checked", true);
                $('.lexic4_23').slice(lexicList[0].lexic23 - 1, lexicList[0].lexic23).prop("checked", true);
                $('.lexic4_34').slice(lexicList[0].lexic34 - 1, lexicList[0].lexic34).prop("checked", true);
                $('.lexic4_24').slice(lexicList[0].lexic24 - 1, lexicList[0].lexic24).prop("checked", true);
                $('.lexic4_14').slice(lexicList[0].lexic14 - 1, lexicList[0].lexic14).prop("checked", true);
                ...
                $('.lexic3_12').slice(lexicList[0].lexic12 - 1, lexicList[0].lexic12).prop("checked", true);
                $('.lexic3_13').slice(lexicList[0].lexic13 - 1, lexicList[0].lexic13).prop("checked", true);
                $('.lexic3_23').slice(lexicList[0].lexic23 - 1, lexicList[0].lexic23).prop("checked", true);
                ...
                $('.lexic2_12').slice(lexicList[0].lexic12 - 1, lexicList[0].lexic12).prop("checked", true);
        ...
        loadMorphologyWord: function (id_word) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    morphemsRecords[0] = morphologyWord.morphems;
                }
            });
        },
        loadMorphologyAnswer: function (id_answer) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    morphemsRecords[1] = morphologyAnswer.morphems;
                }
            });
        },
        loadMorphologyAnswer2: function (id_answer) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    morphemsRecords[2] = morphologyAnswer2.morphems;
                }
            });
        },
        ...
        loadMorphologyAnswer3: function (id_answer) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    morphemsRecords[3] = morphologyAnswer3.morphems;
                }
            });
        },
        ...
        loadMorphologyAnswer4: function (id_answer) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    morphemsRecords[4] = morphologyAnswer4.morphems;
                }
            });
        },
        loadLevel: function (level) {
            var realLevels = document.getElementsByClassName("disabledClass")[0];
            realLevel = realLevels.getAttribute("data-level");
            var id_trs = document.getElementsByClassName("disabledClass")[0];
            id_tr = id_trs.getAttribute("data-id_tr");
            var id_e = document.getElementsByClassName("disabledClass")[0].getAttribute("data-id");
            $.ajax({
                type: 'GET',
                url: '/teacher/GetSyllMirrorWordsByLevel',
                data: {
                    // level: level,
                    id:id_e,
                    level: realLevel,
                ...
                success: function (response) {
                    ...
                    self.loadMorphologyWord(exercises[0].id_word);
                    fillInputs(0, morphologyWord);
                    self.loadMorphologyAnswer(exercises[0].id_answer1);
                    fillInputs(1, morphologyAnswer);
                    ...
                    try {
                        ...
                        if (exercises[step].id_answer2 !== undefined && exercises[step].id_answer2 !== null) {
                            ...
                            $("#btnEx").click();
                            fillInputs(2, morphologyAnswer2);
                            fillInputsLexic(2, morphologyAnswer2, morphologyAnswer, morphologyAnswer3, morphologyAnswer4);
                        }
                    }
                    catch (e) { alert('Ошибка ' + e.name + ":" + e.message + "\n" + e.stack); }
                    ...
                    try {
                        ...
                        if (exercises[step].id_answer3 !== undefined && exercises[step].id_answer3 !== null) {
                            ...
                            $("#btnEx").click();
                            fillInputs(3, morphologyAnswer3);
                            fillInputsLexic(3, morphologyAnswer2, morphologyAnswer, morphologyAnswer3, morphologyAnswer4);
                        }
                    }
                    catch (e) { alert('Ошибка ' + e.name + ":" + e.message + "\n" + e.stack); }
                    ...
                    try {
                        if (exercises[step].id_answer4 !== undefined && exercises[step].id_answer4 !== null) {
                            ...
                            $("#btnEx").click();
                            fillInputs(4, morphologyAnswer4);
                            fillInputsLexic(4, morphologyAnswer2, morphologyAnswer, morphologyAnswer3, morphologyAnswer4);
                        }
                    }
                    catch (e) { alert('Ошибка ' + e.name + ":" + e.message + "\n" + e.stack); }
                }
...
function addWord(id, words, primaries, phonetics, contexts, morphems, morphologyId, bonusVisible, lexics, parts, tr) {
    var m;
    if (id === 0) m = morphologyWord.id;
    if (id === 1) m = morphologyAnswer.id;
    if (id === 2) m = morphologyAnswer2.id;
    if (id === 3) m = morphologyAnswer3.id;
    if (id === 4) m = morphologyAnswer4.id;
    $.ajax({
        type: 'POST',
        url: '/teacher/EditWord',
        data: {
            id_w:m,
            word: words[id],
...
function addLexicAll(lexics, words, parts, wordId, tr, id) {
    var lexId = null;
    try { lexId = lexicList[0].id_lexicAll; } catch (e) { alert('Ошибка ' + e.name + ":" + e.message + "\n" + e.stack); }
    if (lexics[3] !== null) {
        var a = lexics[2];
        lexics[2]= lexics[3];
        lexics[3] = a;
...
function addSyllMirror(words, lexicAllId, wordId,parts,tr,id) {
    ...
    $.ajax({
        type: 'POST',
        url: '/teacher/EditSyllMirror',
        data: {
            id_s: document.getElementsByClassName("disabledClass")[0].getAttribute("data-id"),
            id_word: words1[0],
            id_answer1: words1[1],
            id_answer2: words1[2],
            id_answer3:words1[3],
...
function addTheme(syllMirrorId, parts, tr,level) {
    ...
    for (var i = 0; i < parts1.length; i++) {
        ...
        $.ajax({
            type: 'POST',
            url: '/teacher/EditTheme',
            data: {
                id_flag:i,
                id_e: document.getElementsByClassName("disabledClass")[0].getAttribute("data-id"),
...
function addMorphology(id, parts, morph1, words, primaries, phonetics, contexts, morphems, bonusVisible, lexics, tr) {
    var m;
    if (id === 0) m = morphologyWord.id_morphology;
    if (id === 1) m = morphologyAnswer.id_morphology;
    if (id === 2) m = morphologyAnswer2.id_morphology;
    if (id === 3) m = morphologyAnswer3.id_morphology;
    if (id === 4) m = morphologyAnswer4.id_morphology;
    $.ajax({
        type: 'POST',
        url: '/teacher/EditMorphology',
        data: {
            id_m:m,
            id_part:parts[id],
...
function fillInputs(i, morphologyWord) {
    $(".wordInput")[i].value = morphologyWord.word;
    $(".phoneticInput")[i].value = morphologyWord.phonetic;
    $(".primaryInput")[i].value = morphologyWord.primary_form;
    $(".contextInput")[i].value = morphologyWord.context;
    $(".forMorphii")[i].value = morphologyWord.morphems;
    $(".morphContext")[i].innerHTML = morphologyWord.context;
    var a = $('.chooseLabel').find('input[value = ' + morphologyWord.id_part + ']').slice(i, i + 1).closest('label');
    a.css("background", "linear-gradient(to bottom, rgba(255,255,255,1) 0%,rgba(229,229,229,1) 100%)");
    a.css('background', 'grey');
    $('.chooseLabel').find('input[value = ' + morphologyWord.id_part + ']').slice(i, i + 1).click();
    ...
    var ii = i;
        ...
        var blockWord = $('.morphologyDiv').slice(ii, ii + 1);
            for (var property in morphologyObj) {
                if (morphologyObj.hasOwnProperty(property)) {
                    if (blockWord.find('.js-syllMirror-' + property + '>select').is(":visible"))
                        blockWord.find('.js-syllMirror-' + property + '>select').prop('selectedIndex', morphologyWord[property]);
                }
            }
}

function fillInputsLexic(i, morphologyAnswer2, morphologyAnswer, morphologyAnswer3, morphologyAnswer4) {
    if (i === 2) {
        littlefill(0, morphologyAnswer);
        littlefill(1, morphologyAnswer2);
    }
    if (i === 3) {
        littlefill(2, morphologyAnswer);
        littlefill(3, morphologyAnswer2);
        littlefill(4, morphologyAnswer3);
    }
    if (i === 4) {
        littlefill(5, morphologyAnswer);
        littlefill(6, morphologyAnswer2);
        littlefill(7, morphologyAnswer3);
        littlefill(8, morphologyAnswer4);
    }
}

function littlefill(i, morphologyAnswer) {
    $(".forLexicText")[i].innerHTML = morphologyAnswer.name;
    $(".forLexic")[i].innerHTML = morphologyAnswer.word;
}

function abcdef() {
   document.getElementById("canvas600").style.marginLeft = -3 + "%";
}